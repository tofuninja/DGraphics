...
// TODO remove this compleatly, being replaced by graphics.gui
...
class Panel
{
	...
	final void composit()
	{
		...
		if(img !is null && base !is null && visable)
		{
...
class Button : Panel
{
	...
	public Color color = Color(200,200,200);
...
class checkBox : Panel
{
	...
	public void delegate(bool) onCheck;
	...
	override public void mouseDown(mouseState state) 
	{
		value = !value;
		drawChecked();
		if(onCheck !is null) onCheck(value);
	}
}

class ValueSlider(T = float) : Panel
{
	...
	public void delegate(T) onChange;

	public this(vec2 loc, float length, string text, T left, T right, Panel owner)
	{
	...
	public this(vec2 loc, float length, string text, T left, T right)
	{
	...
	override public void tick() 
	{
		...
		if(down)
		{
			...
			else
			{
				...
				value = cast(T)(l + ((mousex - loc.x - 8) / (size.x-16))*(r - l));
				value = max(l, value);
				value = min(r, value);
				if(onChange !is null) onChange(value);
				drawSlider();
			}
		}
	}
	...
	private void drawSlider()
	{
		...
		float ratio = (value - l)/(cast(float)(r - l));
