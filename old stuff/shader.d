...
// TODO switch uniform usage to UBO!!!! MUST DO THIS!!! or at least at ubo support!
...
public class ShaderProgram
{
	...
	private void getUniforms()
	{
		...
		const GLenum[4] properties = [GL_BLOCK_INDEX, GL_TYPE, GL_NAME_LENGTH, GL_LOCATION];
	...
	private void getInputs()
	{
		...
		const GLenum[4] properties = [GL_TYPE, GL_NAME_LENGTH, GL_LOCATION, GL_ARRAY_SIZE];
...
public class ShaderInput
{
	...
	{
		drawInstanced(vertexOffset, vertexCount,instanceCount);
	}
	
	public void drawInstanced(int start, int count, int instanceCount)
	{
		
		if(indexBufferAttached) 
		{
			drawIndexedInstanced(start, count, instanceCount);
		}
		else
		{
			glUseProgram(program.id);
	...
	private void drawIndexedInstanced(int start, int count, int instanceCount)
	{
		glUseProgram(program.id);
		glBindVertexArray(id);
		glDrawElementsInstanced(GL_TRIANGLES, count, GL_UNSIGNED_INT, cast(void*)(start*uint.sizeof), instanceCount);
		glBindVertexArray(0);
		glUseProgram(0);
	}
	...
	public void attachBufferDiv(V : VertexBuffer!(T), T)(V vbo, int div)
	{
		import std.traits;
		import std.stdio;
		...
		static if(!vbo.isSingleAttrib)
		{
			...
			foreach(mem; __traits(allMembers, T))
			{
				...
				if(input != null)  // Posible for this to not be an error if the input is not active so we will just treat it like nothing happened
				{
					...
					attachAtrib!(mem_T)(input.location, stride, offset, input.type);
					glVertexAttribDivisor(input.location, div);
				}
			}
		}
		else
	...
	private void attachAtrib(T)(int index, int stride, int offset, TypeInfo destType)
	{
		...
		else static if(is(T == mat4))
		{
			...
			glVertexAttribPointer(index    , 4, GL_FLOAT, GL_FALSE, mat4.sizeof, cast(void*) offset                 );
			glVertexAttribPointer(index + 1, 4, GL_FLOAT, GL_FALSE, mat4.sizeof, cast(void*)(offset +   vec4.sizeof));
			glVertexAttribPointer(index + 2, 4, GL_FLOAT, GL_FALSE, mat4.sizeof, cast(void*)(offset + 2*vec4.sizeof));
			glVertexAttribPointer(index + 3, 4, GL_FLOAT, GL_FALSE, mat4.sizeof, cast(void*)(offset + 3*vec4.sizeof));
		}
		else
		{
			static assert(false,"Unsuported Type " ~ T.stringof);
...
private TypeInfo toTypeInfo(GLint gl_type)
{
	...
	switch(gl_type)
	{
		...
		case GL_SAMPLER_CUBE:		return typeid(TextureImageUnit);
