...
// TODO Trash most of this! remove its OGL dependecise and make it just a internal format for mesh data which can be easily converted to VBOs
...
public class mesh
{
	...
	public void generateBuffers()
	{
		...
		vectorsBuffer = new VertexBuffer!vec3(vectors, "pos");
		colorsBuffer = new VertexBuffer!vec3(colors, "col");
		normalsBuffer = new VertexBuffer!vec3(normals, "norm");
		texCordsBuffer = new VertexBuffer!vec2(texCords, "uv");
		indicesBuffer = new IndexBuffer(indices);
...
/**
 * Cunstructs an axis aligned box
 */
public mesh boxMesh()
{
...
/**
 * Cunstructs a cone with no bottom
 */
public mesh coneMesh(int N)
{
	import std.math;
	if(N < 3) N = 3;
	...
	vec3[] vecs = new vec3[vecN];
	vec3[] vcols = new vec3[vecN];
	vec3[] norms = new vec3[vecN];
	vec2[] uvArr = new vec2[vecN];
	uvec3[] index = new uvec3[triN];
	
	for(int i = 0; i < N; i++)
	{
		float r = (i/(cast(float)N))*2.0f*PI;

		vecs[i] = vec3(sin(r), -1, cos(r));
		vcols[i] = vec3(1,1,1);
		norms[i] = vec3(sin(r), 0, cos(r));
		uvArr[i] = vec2(sin(r), cos(r))/2.0f + vec2(0.5f,0.5f);
	}

	vecs[N] = vec3(0,1,0);
	vcols[N] = vec3(1,1,1);
	norms[N] = vec3(0,1,0);
	uvArr[N] = vec2(0.5f, 0.5f);


	for(int i = 0; i < N; i++)
	{
		index[i] = uvec3(i, (i+1)%N, N);
	}
...
public mesh tesPlane(int N)
{
	...
	vec2[] uvArr = new vec2[vecN];
	uvec3[] index = new uvec3[triN];
	int setvert(int x, int y)
	{
		int id = x*N + y;
		vecs[id] = vec3((x)/(cast(float)N-1),0,(y)/(cast(float)N-1));
		vcols[id] = vec3(1,1,1);
		norms[id] = vec3(0,1,0);
		uvArr[id] = vecs[id].xz;
		return id;
	...
	for(int i = 0; i < N-1; i++)
	{
		...
		{
			int v00 = setvert(i,j);
			int v10 = setvert(i+1,j);
			int v01 = setvert(i,j+1);
			int v11 = setvert(i+1,j+1);
			index[tri] = uvec3(v00, v10, v11);
			index[tri+1] = uvec3(v11, v01, v00);
			tri += 2;
		}
	}
