...
public import math.geo.vector;
public import math.geo.matrix;
public import math.geo.quaternion;
...
//
//alias vec2 = matrix!(2,1,float);
//alias vec3 = matrix!(3,1,float);
//alias vec4 = matrix!(4,1,float);
//
...
//    T[4] quat = ypr(0,0,0);
//
//    auto x() { return quat[0]; }
//    auto y() { return quat[1]; }
//    auto z() { return quat[2]; }
//    auto w() { return quat[3]; }
//
//    /// Constructs a quaternion for rotation
//    this(matrix!(3,1,T) axis, T angle)
//    {
...
//    auto getYPR()
//    {
//        T roll  = atan2(2*(quat[3]*quat[0] + quat[1]*quat[2]), 1 - 2*(quat[0]*quat[0] + quat[1]*quat[1]));
//        T pitch = asin(2*(quat[3]*quat[1] - quat[2]*quat[0]));
//        T yaw   = atan2(2*(quat[3]*quat[2] + quat[0]*quat[1]), 1 - 2*(quat[1]*quat[1] + quat[2]*quat[2]));
//        return matrix!(3,1,T)(yaw, pitch, roll);
//    }
//
//    private static auto ypr(T yaw, T pitch, T roll)
//    {
//        T[4] q;
...
//    }
//
//    auto opBinary(string op : "*",T)(QuaternionT!T r) 
//    {
//        auto q = this;
//        QuaternionT!T ret;
...
//    }
//}
//// TODO ok, its finaly come, this is no longer good enough, going to have to rewrite almost all of this
//// The current is just too damn slow.... 
//// Going to try and use simd, hopefully that will make every thing fast as hell 
//
//
//
///**
...
//            static assert(opBinCheck!(T, T2, "/"),"opBinary(/) is not defined between " ~ T.stringof ~ " and " ~ T2.stringof);
...
//    {
//        return verticalCat!(m,n,m2,n2,T)(this,rhs);
//    }
//    
//    static if(isVector)
//    {
...
//        {
//            return verticalCat!(m,T)(this,rhs);
//        }
//
//        auto opBinaryRight(string op : "~")(T lhs) 
//        {
//            return verticalCat!(m,T)(lhs,this);
//        }
//    }
//
//    /// Casting op
...
//}
//
//private template opBinCheck(T1, T2, string op)
//{
//    static if(__traits(compiles, (T1 a, T2 b) {mixin("return a" ~ op ~ "b;");}))
//        enum opBinCheck = true;
//    else
//        enum opBinCheck = false;
//}
//
///**
...
//
///**
//* Vertical cat for vectors and single values
//*/
//public auto verticalCat(int len, T)(matrix!(len,1,T) a, T value) 
//{
//    alias rtnT = matrix!(len + 1, 1, T);
//    rtnT rtn;
//
//    for(int i = 0; i < len; i++)
//    {
...
//}
//
//public auto verticalCat(int len, T)(T value, matrix!(len,1,T) a) 
//{
//    alias rtnT = matrix!(len + 1, 1, T);
//    rtnT rtn;
//
...
//{
//    auto v = matrix!(4,1,T)(q.quat);
//    v = normalize(v);
//    return QuaternionT!T(v.m_data);
//}
//
///**
// * Constructs a rotation matrix from a Quaternion
// */
//auto rotationMatrix(T=float)(QuaternionT!T q)
//{
//    static assert(__traits(compiles, cast(real)T.init), "No cast from " ~ T.stringof ~ " to real");
//    static assert(__traits(compiles, cast(T)real.init), "No cast from real to " ~ T.stringof);
//    static assert(isNumeric!T, T.stringof ~ " is not numaric");
...
//{
//    return rotationMatrix(QuaternionT!T(axis, angle));
//}
//
///**
...
//{
//    return rotationMatrix(QuaternionT!T(ypr));
//}
//
///**
...
//}
//
//auto modelMatrix(vec3 translation, Quaternion quaternion, vec3 scale)
//{
//    return translationMatrix(translation)*rotationMatrix(quaternion)*scalingMatrix(scale);
//}
//
//
//
...
//
//
//// TODO need some unit tests for this shit!!!!
//unittest
//{
//    assert(vec3(1,2,3) ~ 4 == vec4(1,2,3,4));
//    assert(0 ~ vec3(1,2,3) == vec4(0,1,2,3));
//    assert(vec2(0,1) ~ vec2(2,3) == vec4(0,1,2,3));
//
//    assert(vec3(1,2,3) + vec3(1,2,3) == vec3(2,4,6));
//    assert(vec3(1,2,3) - vec3(1,2,3) == vec3(0,0,0));
