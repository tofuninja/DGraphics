...
void startTrackingAllocs(File logFile = stdout) {
	...
	*pproxy = &myProxy;  
	logFile.writeln("GC tracking started");
}

void stopTrackingAllocs() {
	*pproxy = null; 
	allocs_file.writeln("GC tracking ended");
}

auto GCTracker(File logFile = stdout) // RAII version: starts tracking now, ends it when returned value dies
{
	struct S {
